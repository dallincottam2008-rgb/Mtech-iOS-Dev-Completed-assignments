...
print(calculateArea(x: 3.0, y: 7.0))
print(calculateArea(x: -2.4, y: -4.5))
...
func createUser() -> User? {
    guard let firstNameUnwrap = firstNameTextField.text, let lastNameUnwrap = lastNameTextField.text, let ageUnrwap = ageTextField.text else {
        return nil
    }
    let returnedUser = User(firstName: firstNameUnwrap, lastName: lastNameUnwrap, age: ageUnrwap)
    return returnedUser
}
...
var test = createUser()
if let userFirstName = test?.firstName, let userLastName = test?.lastName, let userAge = test?.age {
