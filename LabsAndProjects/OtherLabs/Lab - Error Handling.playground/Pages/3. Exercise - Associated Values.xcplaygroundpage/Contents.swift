...
do {
    ...
    print("The network service is unavailable")
} catch CommunicationError.invalidMessage(let messageError) {
    print("Error, Invalid message")
} catch CommunicationError.timedOut(waitTime: 5) {
    print("Timed out")
} catch CommunicationError.cancelledByUser {
    print("Cancelled by user")
} catch CommunicationError.rejectedByServer(reason: "Bot") {
    print("rejected by sever")
} catch {
    print("Unkown error")
}
