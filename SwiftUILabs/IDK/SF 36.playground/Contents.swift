...
func insertionSort(_ array: [Int]) {
    ...
    for num in 0..<totalCount{
        
        let curretnNum = returnArray[num]
        
        guard curretnNum >= 0 else { return }
        
        while curretnNum < returnArray[num - 1] {
            returnArray.swapAt(curretnNum, returnArray[num - 1])
            print(returnArray)
        }
